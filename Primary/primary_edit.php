...
.bg-img::after {
    ...
    $aadhaar = $row2['aadhaar'];
        ...
        if ($stud_living == "Guardian") {
            $query1  = mysqli_query($conn, "SELECT * FROM `tbl_guardian` WHERE email='$stud_email' AND admission_type='Primary'");
        ...
        if ($sibling == "Yes") {
            $size=0;
            $query2  = mysqli_query($conn, "SELECT * FROM `tbl_sibling` WHERE email='$stud_email' AND admission_type='Primary'");
...
$transfer_img_name = $_FILES['transfer_certificate']['name'];
            ...
            if ($er == 0) {
                ...
                $Updatequery1 =mysqli_query($conn,"UPDATE `tbl_primary` SET `passed_class`='$passed_class',`second_language`='$second_language',`third_language`='$third_language',`prev_school_name`='$prev_school_name',`prev_school_affliated`='$prev_school_affliated',`fav_subject`='$fav_subject',`study_habits`='$study_habits' WHERE email='$email'");
                $Updatequery2 = mysqli_query($conn, "UPDATE `tbl_family` SET `father_name`='$father_name',`father_qualification`='$father_qualification',`father_profession`='$father_profession',`father_phonenumber`='$father_phonenumber',`father_email`='$father_email',`father_annualincome`='$father_annualincome',`mother_name`='$mother_name',`mother_qualification`='$mother_qualification',`mother_profession`='$mother_profession',`mother_phonenumber`='$mother_phonenumber',`mother_email`='$mother_email',`mother_annualincome`='$mother_annualincome',`role_of_parent`='$role_of_parent' WHERE email='$email' AND admission_type='Primary'");
                if (is_uploaded_file($stud_img_loc) == 1) {
                    unlink($stud_photo);
                ...
                if (is_uploaded_file($transfer_img_loc) == 1) {
                    ...
                    $tc = mysqli_query($conn, "UPDATE `tbl_primary` SET `transfer_certificate`='" . mysqli_real_escape_string($conn, $transfer_img_path) . "' WHERE email='$email'");
                    if ($tc) {
                ...
                if (is_uploaded_file($aadhaar_img_loc) == 1) {
                    unlink($aadhaar);
                    $a = mysqli_query($conn, "UPDATE `tbl_primary` SET `aadhaar`='" . mysqli_real_escape_string($conn, $aadhaar_img_path) . "' WHERE email='$email'");
                    if ($a) {
                ...
                if ($Updatequery && $Updatequery1 && $Updatequery2) {
                    if ($sibling == "No") {
                        ...
                        if ($size != 0) {
                            for ($i = 0; $i < $size; $i++) {
                                $Updatequery3 = mysqli_query($conn, "UPDATE `tbl_sibling` SET `sibling_name`='$sibling_name[$i]',`sibling_age`='$sibling_age[$i]',`sibling_class`='$sibling_class[$i]' WHERE email='$email' AND admission_type='Primary' AND `sibling_name`= '$siblingname[$i]' ");
                            }
                        }
                    } 
                    ...
                    if ($stud_living == "Guardian") {
                        $Updatequery4 = mysqli_query($conn, "UPDATE `tbl_guardian` SET `guardian_name`='$guardian_name',`guardian_address`='$guardian_address',`guardian_email`='$guardian_email',`guardian_contact`='$guardian_phonenumber' WHERE email='$email' AND admission_type='Primary' ");
                    } else {
                                ...
                                <div class="section-title">
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="2"><select name="class" class="form-control" id="class" onclick="getpassedclass();" value="<?php print $class; ?>">
                                                ...
                                                <?php
                                                $SELECTquery5 = mysqli_query($conn, "SELECT primary_class FROM tbl_option WHERE primary_class != '$class' AND  primary_class IS NOT NULL");
                                                while ($rw5 = mysqli_fetch_array($SELECTquery5)) {
                                                ?>
                                                    <option value="<?php echo $rw5['primary_class']; ?>"><?php echo $rw5['primary_class']; ?></option>
                                                <?php }
                                                ?>
                                            </select></td>
                                    </tr>
