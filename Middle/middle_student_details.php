...
if (isset($_POST['submit'])) {
    ...
    if ($sport != "") {
        ...
        if (!preg_match("/^[a-zA-Z ]*$/", $sport)) {
            $er++;
            $esport = "*Only letters, white space  are allowed";
        }
    }
    ...
    if ($er == 0) {
        ...
        $sql1 = 'INSERT INTO `tbl_middle`(`email`, `passed_class`, `second_language`,`third_language`, `prev_school_name`, `prev_school_affliated`, `fav_subject`, `study_habits`, `sport`, `co_curricular`) VALUES (
        ...
        "' . mysqli_real_escape_string($conn, $co_curricular) . '"
        )';
        ...
        if ($query && $query1) {
            ...
            $_SESSION['location'] = "middle_family_details.php";
            $email = "";
            $gender = "";
            $placeofbirth = "";
            $religion = "";
                ...
                <div class="col-md-10">
                    <table class="table table-bordered table-striped">
                        <tbody>
                            ...
                            <tr>
                                <td>Admission for the grade <span class="require">*</span><span class="error"><br>
                                        <?php print $eclass; ?></span></td>
                                <td><select name="class" class="form-control" id="class" onclick="getpassedclass();" value="<?php print $class; ?>">
                                        <?php if ($class == "") { ?>
                                            <option value="">Select Class</option>
                                            <?php $query = mysqli_query($conn, "SELECT middle_class FROM tbl_option WHERE middle_class IS NOT NULL");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['middle_class']; ?>"><?php echo $row1['middle_class']; ?></option>
                                            <?php }
                                        } else {
                                            ?>
                                            <option value="<?php echo $class; ?>"><?php echo $class; ?></option>
                                            <?php
                                            $query = mysqli_query($conn, "SELECT middle_class FROM tbl_option WHERE middle_class != '$class'");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['middle_class']; ?>"><?php echo $row1['middle_class']; ?></option>
                                        <?php }
                            ...
                            <tr>
                                <td>Second Language Prefered <span class="require">*</span><span class="error"><br>
                                        <?php print $esecond_language; ?></span></td>
                            ...
                            <tr>
                                <td>Third Language Prefered<span class="error"><br>
                            ...
                            <tr>
                                <td>Active participation in the sport of<span class="error"><br>
                                        <?php print $esport; ?></span></td>
                                <td><input type="text" class="form-control" name="sport" value="<?php print $sport; ?>" ></td>
                            </tr>

                            <tr>
                                <td>Achievements in co-curricular activities<span class="error"><br>
                                        <?php print $errco_curricular; ?></span></td>
                                <td><input type="text" class="form-control" name="co_curricular" value="<?php print $co_curricular; ?>" ></td>
                            </tr>
