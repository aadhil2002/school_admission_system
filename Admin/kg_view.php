...
$query = mysqli_query($conn, "SELECT * FROM tbl_student A INNER JOIN tbl_family B ON A.email= B.email WHERE A.email='$stud_email'");
    ...
    $stud_photo = "../KG/".$row1['student_photo'];
$father_photo = "../KG/".$row1['father_photo'];
$mother_photo = "../KG/".$row1['mother_photo'];
$birth_certificate = "../KG/".$row1['birth_certificate'];
$wellness_certificate="../KG/".$row1['wellness_certificate'];

        if ($stud_living == "Guardian") {
            $query1  = mysqli_query($conn, "SELECT * FROM `tbl_guardian` WHERE email='$email' AND admission_type='KG'");
        ...
        if ($sibling == "Yes") {
            $size=0;
            $query2  = mysqli_query($conn, "SELECT * FROM `tbl_sibling` WHERE email='$email' AND admission_type='KG'");
