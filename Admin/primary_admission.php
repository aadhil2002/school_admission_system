...
$i = 1;
$query = mysqli_query($conn,"SELECT stud_name,birthdate,age,email,contact,class FROM tbl_student WHERE `payment_status`='1' AND `admission_type`='Primary' ORDER BY admission_status ASC");
?>
<!DOCTYPE HTML>
<html>
...
PRIMARY ADMISSION
				...
				?>
						<tr>
                            ...
                            <td><?php
							 ...
							 {
								?>
							<a href="primary_admission.php?accept=<?php echo $row["email"];?>&class=<?php echo $row['class']; ?>"> <button class="btn btn-success">ACCEPT</button> </a><a href="primary_admission.php?reject=<?php echo $row["email"];?>"> <button class="btn btn-danger">REJECT </button></a></td>
                          <?php   
						}
				...
				<?php   
				   ...
				   ?>
						<td><a href="primary_view.php?email=<?php echo $row["email"];?>"><button class="btn btn-info">VIEW</button> </a></td>
						</tr>
				<?php
					}
				}
