...
$amount=$am['admission_fee'];
    if(isset($_POST['cancel']))
    {
        ...
        $class_secondary=$class['class'];
            $query=mysqli_query($conn,"SELECT `aval_count`FROM `tbl_class_count` WHERE class='".$class_secondary."'");
$count=mysqli_fetch_assoc($query);
$c=$count['aval_count'];
$c=$c+1;
mysqli_query($conn,"UPDATE `tbl_class_count` SET `aval_count`='".$c."' WHERE class='".$class_secondary."'");
        }
