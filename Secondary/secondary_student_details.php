...
if (isset($_POST['submit'])) {
    ...
    if ($er == 0) {
        ...
        $sql1 = 'INSERT INTO `tbl_secondary`(`email`, `passed_class`, `second_language`,`prev_school_name`, `prev_school_affliated`, `fav_subject`, `study_habits`, `sport`, `co_curricular`) VALUES (
        ...
        if ($query && $query1) {
            ...
            $_SESSION['location'] = "secondary_family_details.php";
            $email = "";
            $gender = "";
            $placeofbirth = "";
            $religion = "";
                            ...
                            <tr>
                                ...
                                <td><select name="class" class="form-control" id="class" onclick="getpassedclass();" value="<?php print $class; ?>">
                                        <?php if ($class == "") { ?>
                                            ...
                                            <?php $query = mysqli_query($conn, "SELECT secondary_class FROM tbl_option WHERE secondary_class IS NOT NULL");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['secondary_class']; ?>"><?php echo $row1['secondary_class']; ?></option>
                                            <?php }
                                        } else {
                                            ?>
                                            <option value="<?php echo $class; ?>"><?php echo $class; ?></option>
                                            <?php
                                            $query = mysqli_query($conn, "SELECT secondary_class FROM tbl_option WHERE secondary_class != '$class'");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['secondary_class']; ?>"><?php echo $row1['secondary_class']; ?></option>
                                        <?php }
