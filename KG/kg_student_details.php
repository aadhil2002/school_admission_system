...
if (isset($_POST['submit'])) {
    ...
    if ($er == 0) {
        ...
        if ($query) {
            ...
            $_SESSION['location'] = "kg_family_details.php";
            $email = "";
            $gender = "";
            $placeofbirth = "";
            $religion = "";
                    ...
                    <table class="table table-bordered table-striped">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><select name="class" class="form-control"  value="<?php print $class; ?>">
                                        <?php if ($class == "") { ?>
                                            <option value="">Select Class</option>
                                            <?php $query = mysqli_query($conn, "SELECT KG_class FROM tbl_option WHERE KG_class IS NOT NULL");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['KG_class']; ?>"><?php echo $row1['KG_class']; ?></option>
                                            <?php }
                                        } else {
                                            ?>
                                            <option value="<?php echo $class; ?>"><?php echo $class; ?></option>
                                            <?php
                                            $query = mysqli_query($conn, "SELECT KG_class FROM tbl_option WHERE KG_class != '$class'");
                                            while ($row1 = mysqli_fetch_array($query)) {
                                            ?>
                                                <option value="<?php echo $row1['KG_class']; ?>"><?php echo $row1['KG_class']; ?></option>
                                        <?php }
