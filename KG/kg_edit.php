...
<?php
    ...
    include '../includes/connect.php';
    $query = mysqli_query($conn, "SELECT * FROM tbl_student A INNER JOIN tbl_family B ON A.email= B.email WHERE A.email='$stud_email'");
    ...
    if ($stud_living == "Guardian") {
        $query1  = mysqli_query($conn, "SELECT * FROM `tbl_guardian` WHERE email='$stud_email' AND admission_type='KG'");
    ...
    if ($sibling == "Yes") {
        $size = 0;
        $query2  = mysqli_query($conn, "SELECT * FROM `tbl_sibling` WHERE email='$stud_email' AND admission_type='KG'");
        ...
        if ($er == 0) {
            ...
            if ($Updatequery && $Updatequery1) {
                if ($sibling == "No") {
                    ...
                    if ($size != 0) {
                        for ($i = 0; $i < $size; $i++) {
                            $Updatequery2 = mysqli_query($conn, "UPDATE `tbl_sibling` SET `sibling_name`='$sibling_name[$i]',`sibling_age`='$sibling_age[$i]',`sibling_class`='$sibling_class[$i]' WHERE email='$email' AND admission_type='KG' AND `sibling_name`= '$siblingname[$i]' ");
                        }
                    }
                }
                ...
                if ($stud_living == "Guardian") {
                    $Updatequery3 = mysqli_query($conn, "UPDATE `tbl_guardian` SET `guardian_name`='$guardian_name',`guardian_address`='$guardian_address',`guardian_email`='$guardian_email',`guardian_contact`='$guardian_phonenumber' WHERE email='$email' AND admission_type='KG' ");
                } else {
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2"><select name="class" class="form-control" id="class" onclick="getpassedclass();" value="<?php print $class; ?>">
                                            ...
                                            <?php
                                            $SELECTquery5 = mysqli_query($conn, "SELECT KG_class FROM tbl_option WHERE KG_class != '$class' AND  KG_class IS NOT NULL");
                                            while ($rw5 = mysqli_fetch_array($SELECTquery5)) {
                                            ?>
                                                <option value="<?php echo $rw5['KG_class']; ?>"><?php echo $rw5['KG_class']; ?></option>
                                            <?php }
                                            ?>
                                        </select></td>
                                </tr>
